...
describe('core event emitter', () => {
  test('action emits started → state.changed → completed', () => {
    ...
    const store = createStore(
      ({ set, get }) => ({ count: 0, double: () => get().count * 2, inc: () => set((s) => ({ count: s.count + 1 })) }),
  ...
  test('resource emits loading → success', async () => {
    const events: StoreEvent[] = [];
    const store = createStore(
      ({ ctx }) => ({ user: ctx.resource<string>({ name: 'user', fetch: async () => 'ok' }) }),
      { plugins: [capturePlugin(events)] },
    );
  ...
  test('mutation emits started → success', async () => {
    const events: StoreEvent[] = [];
    const store = createStore(
      ({ ctx }) => ({ save: ctx.mutation<number, number>({ name: 'save', run: async (n) => n * 2 }) }),
      { plugins: [capturePlugin(events)] },
    );
    await store.save(5);
