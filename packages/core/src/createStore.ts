...
export function buildStore<T>(
	...
	const set: Setter<T> = (partial) => {
		...
		if (emitter.size()) {
			...
			for (const key of Object.keys(next as Dict)) {
				if (!Object.is((next as Dict)[key], prev[key]))
					patch[key] = (next as Dict)[key];
			}
			emitter.emit({
				type: "state.changed",
				payload: { prev, next: merged, patch },
			});
		}
	};
	...
	const ctx: StoreContext<T> = {
		...
		resource: ((options: never) =>
			createResource(
				...
				options,
				emitter.emit,
			)) as StoreContext<T>["resource"],
		mutation: ((options: never) =>
			createMutation(
				adapter,
				options,
				emitter.emit,
			)) as StoreContext<T>["mutation"],
	};
	...
	initialState = clone(
		(config.hydrate !== undefined ? config.hydrate : stateData) as Dict,
	);
	...
	if (config.persist) {
		...
		if (factory) {
			const persistConfig =
				typeof config.persist === "object" ? config.persist : {};
...
export function getStoreDef<T>(store: Store<T>): StoreDef<T> | undefined {
	return (store as Record<symbol, unknown>)[STORE_DEF] as
		| StoreDef<T>
		| undefined;
}
