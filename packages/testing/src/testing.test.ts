...
describe('createTestStore', () => {
  ...
  test('mockResource overrides a resource', () => {
    const usersStore = createStore(
      ({ get, ctx }) => ({
        selectedId: '1',
        user: ctx.resource<{ id: string; name: string }>({ fetch: async () => ({ id: get().selectedId, name: 'real' }) }),
      }),
      { name: 'users' },
    );
...
describe('mocks', () => {
  test('mockMutation records calls + returns output', async () => {
    const save = mockMutation<{ name: string }, { ok: boolean }>({ output: { ok: true } });
...
describe('flush', () => {
  ...
  test('flushResources settles a real fetch', async () => {
    const usersStore = createStore(
      ({ ctx }) => ({
        user: ctx.resource<string>({ autoRun: true, fetch: async () => 'loaded' }),
      }),
      { name: 'u2' },
    );
    const store = createTestStore(usersStore);
...
describe('createLinkedTestStore', () => {
  test('builds a fresh linked instance', () => {
    const a = createStore(({ set }) => ({ n: 1, setN: (x: number) => set({ n: x }) }), { name: 'a' });
    const b = createStore(({ set }) => ({ m: 2, setM: (x: number) => set({ m: x }) }), { name: 'b' });
    const sum = linkedStore({ a, b }, ({ stores }) => ({ total: () => stores.a.n() + stores.b.m() }), { name: 'sum' });
