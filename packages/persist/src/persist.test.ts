...
const counter = (storage: StorageAdapter, extra = {}) =>
  createStore(({ set }) => ({ count: 0, inc: () => set((s) => ({ count: s.count + 1 })) }), {
    name: 'counter',
...
describe('persist plugin', () => {
  test('saves selected state on change', () => {
    ...
    store.inc();
    flushSync();
    expect(JSON.parse(storage.getItem('counter')!)).toEqual({ v: 0, s: { count: 2 } });
  });
  ...
  test('partial select persists only chosen keys', () => {
    ...
    const store = createStore(
      ...
      { name: 'cart', plugins: [persist({ key: 'cart', storage, select: (s) => ({ items: (s as { items: number[] }).items }) })] },
    );
    store.add(1);
    flushSync();
    expect(JSON.parse(storage.getItem('cart')!).s).toEqual({ items: [1] });
  });
    ...
    const store = createStore(({ set }) => ({ count: 0, inc: () => set((s) => ({ count: s.count + 1 })) }), {
      ...
      plugins: [
        persist({
          ...
          storage,
          version: 2,
          migrate: (old) => ({ count: (old as { count: number }).count * 10 }),
        }),
      ],
    });
  ...
  test('persist: {...} shorthand works once @gehu/persist is imported', () => {
    ...
    store.inc();
    flushSync();
    expect(JSON.parse(storage.getItem('sc')!)).toEqual({ v: 0, s: { count: 1 } });
  });
