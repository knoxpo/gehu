...
|---|---|---|
| `@gehu-js/core` | 4–8 KB | ~2–3 KB |
| `@gehu-js/angular` | 2–5 KB | ~2.4 KB |
| `@gehu-js/persist` | 1.5–3 KB | ~1.2 KB |
| `@gehu-js/devtools` | 2–5 KB (bridge only) | ~0.9 KB |
| `@gehu-js/testing` | 2–4 KB | ~1.6 KB |
...
- **`sideEffects: false`** on every package (except the two that register a
  config shorthand — `@gehu-js/persist` and `@gehu-js/devtools` mark only their
  `dist/index.js`).
...
- Import only what you use: `@gehu-js/core` alone is enough for a working store.
